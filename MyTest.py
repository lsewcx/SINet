...
parser.add_argument("--model_path", type=str, default="SINet_best_1.pth")
parser.add_argument("--test_save", type=str, default="./Result/2020-CVPR-SINet-New/")
parser.add_argument(
    "--image_root",
    type=str,
    default="COD10K/Imgs/",
    help="the root directory of test images",
)
parser.add_argument(
    "--gt_root",
    type=str,
    default="COD10K/GT/",
    help="the root directory of ground truth images",
)
...
for dataset in ["COD10K"]:
    ...
    test_loader = test_dataset(
        image_root=opt.image_root, gt_root=opt.gt_root, testsize=opt.testsize
    )
    img_count = 1
    for iteration in range(test_loader.size):
        ...
        # 评估
        mae = eval_mae(
            torch.from_numpy(cam).to(device), torch.from_numpy(gt).to(device)
        )
        total_mae += mae.item()
        total_images += 1
        # 粗略评分
        print(
            "[Eval-Test] Dataset: {}, Image: {} ({}/{}), MAE: {}".format(
                dataset, name, img_count, test_loader.size, mae
            )
        )
        img_count += 1
